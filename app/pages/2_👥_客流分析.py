...
from app.viz_theme import (
    ...
    inject_global_css, current_palette, apply_theme,
)
...
kpi = (
    ...
    + stat_card_html("雨天均客流", f"{weather['雨']:,.0f}",
                     ...
                     accent=P["cat_red"])
    + stat_card_html("节假日均客流", f"{holiday['节假日']:,.0f}",
                     f"↑ 节假日增 {hol_boost:.0f}%", delta_good=True,
                     accent=P["cat_gold"])
    + stat_card_html("工作日均客流", f"{holiday['工作日']:,.0f}", "基准线",
...
if st.button("生成客流人群画像", type="primary", key="gen_profile"):
    with st.spinner("AI 生成客流画像中..." if is_llm_ready() else None):
        profile = generate_traffic_profile(profile_summary, use_llm=is_llm_ready())
